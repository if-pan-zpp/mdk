...
namespace mdk {
    ...
    public:
        void eval(BaseState const& state, BaseDiff& update) const {
            for (auto const& intv: ranges) {
                for (int i = intv.lower(); i < intv.upper(); ++i) {
                    ...
                    if (r12_x_r23_norm != 0.0) {
                        ...
                        bondAngleTerm(i, theta, update.V, dV_dtheta);

                        update.F[i-1] -= dV_dtheta * dtheta_dr1;
                        update.F[i] -= dV_dtheta * dtheta_dr2;
                        update.F[i+1] -= dV_dtheta * dtheta_dr3;
                    }
                }
            }
        }
